...
const DiscussionCommentCard = ({ data, uid }) => {
...
const editVote = async (upvotes, id, setUpvoted, discussion_id, uid) => {
  const discussionsRef = firebase
    .firestore()
  ...
  if (upvotes.includes(uid)) {
    ...
    await discussionsRef.doc(id).update({ upvotes: new_upvotes });
    setUpvoted(false);
  } else {
